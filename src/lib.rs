...
impl Universe {
    ...
    pub fn new() -> Universe {
        ...
        // let scans = get_input().lines().iter().map(|input| {
        for input in get_input().lines() {
            ...
            scans.push(
            if c1 == "x" {
                ClayScan { x_range: c2..=c2, y_range: c4..=c5 }
            } else {
                ClayScan { x_range: c4..=c5, y_range: c2..=c2 }
            });
        }
        ...
        let mut cells = vec![Cell::Sand; width * height]; //) .map(|_| { Cell::Sand }).collect();

        for scan in &scans {
            for y in scan.y_range.clone() {
                for x in scan.x_range.clone() {
                    cells[y * width + x] = Cell::Clay;
                }
            }
        }
